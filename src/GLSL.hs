{-# LANGUAGE OverloadedStrings, QuasiQuotes, ImportQualifiedPost, FlexibleInstances #-}
...
instance ToSDF SDF where
    sdf SDFNone = "" 
    sdf (SDFShape s) = 
        case s of
            Circle p r -> [i|circle(#{o}, #{sdf p}, #{r})|]
    sdf (SDFOp op) = 
        case op of
            Union a b -> [i|min(#{sdf a}, #{sdf b})|]
            Subtraction a b -> [i|max(#{sdf a}, -#{sdf b})|]
